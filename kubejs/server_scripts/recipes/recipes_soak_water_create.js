...
ServerEvents.recipes(event => {
    ...
    for (let i = 0; i < waters.length; i++) {
        let w = waters[i]
        try {
            // 输入：水 1000mb + 额外流体（如牛奶） + 物品原料列表
            let inputs = [Fluid.of('minecraft:water', 1000)]
            if (w.extraFluids && w.extraFluids.length > 0) {
                for (let k = 0; k < w.extraFluids.length; k++) {
                    let f = w.extraFluids[k]
                    inputs.push(Fluid.of(f.id, f.amount | 0))
                }
            }
            ...
            if (w.extraFluids && w.extraFluids.length > 0) {
                let parts = []
                for (let k = 0; k < w.extraFluids.length; k++) parts.push(w.extraFluids[k].id + '(' + w.extraFluids[k].amount + 'mb)')
                extraDesc = ' + ' + parts.join(',')
            }
            console.log("[SOAK-WATER-CREATE] 注册搅拌配方: " + w.getFluidId() + "(1000mb) <- water(1000mb)" + extraDesc + " + [" + w.ingredients.join(', ') + "]")
        } catch (e) {
